...
{
    /// <summary>
    /// The <see cref="EventLogWarningType"/> MEF Part is used to log warning message types to windows EventLog.
    /// </summary>
    [ExportLoggerOperationType(MessageType = MessageTypeEnum.Warning, LoggerType = LoggerTypeEnum.EventLog)]
    class EventLogWarningType : ILoggerOperationType
    {
        ...
        public void WriteMessageType(string message)
        {
            try
            {
                ...
                evt.WriteEntry(message, EventLogEntryType.Warning);
            }
            catch (Exception)
            {
                // Empty
